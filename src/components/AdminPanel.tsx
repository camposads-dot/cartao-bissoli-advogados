...
export const AdminPanel: React.FC = () => {
  ...
  const [valorRecompensaTipoAcao, setValorRecompensaTipoAcao] = useState<number>(500);
  const [editingTipoAcao, setEditingTipoAcao] = useState<TipoAcao | null>(null);
  ...
  const handleCreateTipoAcao = (e: React.FormEvent) => {
    ...
    const val = Number(valorRecompensaTipoAcao) || currentConfig.valorPadraoCupom || 500;
    apiStore.saveTipoAcao(nomeTipoAcao.trim(), val);
    setTipoMsg(`Tipo de Ação '${nomeTipoAcao}' criado com sucesso com recompensa de R$ ${val.toFixed(2)}!`);
    setNomeTipoAcao('');
    setValorRecompensaTipoAcao(500);
    setTimeout(() => setTipoMsg(''), 3000);
    auth.refreshData();
  };

  const handleUpdateTipoAcao = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingTipoAcao || !editingTipoAcao.nome.trim()) return;

    try {
      const val = Number(editingTipoAcao.valorRecompensa) || 0;
      apiStore.updateTipoAcao(editingTipoAcao.id, {
        nome: editingTipoAcao.nome.trim(),
        valorRecompensa: val,
        ativo: editingTipoAcao.ativo,
      });

      setTipoMsg(`Tipo de Ação '${editingTipoAcao.nome}' atualizado com sucesso!`);
      setEditingTipoAcao(null);
      setTimeout(() => setTipoMsg(''), 3000);
      auth.refreshData();
    } catch (err: any) {
      alert(err.message || 'Erro ao atualizar Tipo de Ação');
    }
  };

  const handleDeleteTipoAcao = (tipo: TipoAcao) => {
    if (window.confirm(`Tem certeza que deseja excluir o Tipo de Ação '${tipo.nome}'?`)) {
      try {
        apiStore.deleteTipoAcao(tipo.id);
        setTipoMsg(`Tipo de Ação '${tipo.nome}' excluído com sucesso!`);
        setTimeout(() => setTipoMsg(''), 3000);
        auth.refreshData();
      } catch (err: any) {
        alert(err.message || 'Erro ao excluir Tipo de Ação');
      }
    }
  };
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-4 sm:space-y-8">
      ...
      {activeTab === 'tipos_acao' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xs space-y-4">
            ...
            <form onSubmit={handleCreateTipoAcao} className="space-y-4 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">
              ...
              <div>
                <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  Valor da Recompensa da Indicação (R$) *
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    min="0"
                    step="10"
                    value={valorRecompensaTipoAcao}
                    onChange={(e) => setValorRecompensaTipoAcao(Number(e.target.value))}
                    placeholder="500.00"
                    className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold text-sm"
                  />
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Valor do cupom concedido ao cliente quando a indicação desta ação fechar contrato.
                </p>
              </div>

              <button
                type="submit"
                className="w-full py-2.5 rounded-xl font-semibold bg-indigo-600 hover:bg-indigo-700 text-white shadow-xs flex items-center justify-center gap-2"
              >
                <PlusCircle className="w-4 h-4" />
                <span>Cadastrar Tipo de Ação</span>
              </button>
            </form>
          </div>
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {tiposAcao.map((t) => {
                const valRec = t.valorRecompensa ?? currentConfig.valorPadraoCupom;
                return (
                  <div
                    key={t.id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 text-xs gap-2"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-900 dark:text-white text-sm">{t.nome}</span>
                        <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                          R$ {valRec.toFixed(2)}
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-400 mt-0.5">
                        Recompensa por indicação convertida
                      </p>
                    </div>

                    <div className="flex items-center space-x-2 shrink-0 self-end sm:self-auto">
                      <button
                        onClick={() => setEditingTipoAcao(t)}
                        className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-indigo-600 dark:text-indigo-400 transition-colors"
                        title="Editar Ação"
                      >
                        <Edit2 className="w-3.5 h-3.5" />
                      ...
                      <button
                        ...
                        className="px-2 py-1 rounded-lg text-xs font-medium border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800"
                        title="Alternar Status"
                      >
                        {t.ativo ? (
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold">Ativo</span>
                        ) : (
                          <span className="text-slate-400 font-bold">Inativo</span>
                        )}
                      </button>

                      <button
                        onClick={() => handleDeleteTipoAcao(t)}
                        className="p-1.5 rounded-lg border border-rose-200 dark:border-rose-900/50 hover:bg-rose-50 dark:hover:bg-rose-950/50 text-rose-600 dark:text-rose-400 transition-colors"
                        title="Excluir Ação"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
      ...
      {/* EDIT TIPO ACAO MODAL */}
      {editingTipoAcao && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 border border-slate-200 dark:border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Edit2 className="w-4 h-4 text-indigo-600" />
              Editar Ação Jurídica: {editingTipoAcao.nome}
            </h3>

            <form onSubmit={handleUpdateTipoAcao} className="space-y-4 text-xs">
              <div>
                <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  Nome da Ação *
                </label>
                <input
                  type="text"
                  required
                  value={editingTipoAcao.nome}
                  onChange={(e) => setEditingTipoAcao({ ...editingTipoAcao, nome: e.target.value })}
                  className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-medium text-sm"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    min="0"
                    step="10"
                    value={editingTipoAcao.valorRecompensa ?? currentConfig.valorPadraoCupom}
                    onChange={(e) =>
                      setEditingTipoAcao({
                        ...editingTipoAcao,
                        valorRecompensa: Number(e.target.value),
                      })
                    }
                    className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold text-sm"
                  />
              ...
              <div>
                ...
                <select
                  value={editingTipoAcao.ativo ? 'true' : 'false'}
                  onChange={(e) => setEditingTipoAcao({ ...editingTipoAcao, ativo: e.target.value === 'true' })}
              ...
              <div className="flex justify-end space-x-2 pt-2">
                <button
                  type="button"
                  onClick={() => setEditingTipoAcao(null)}
                  className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 font-medium"
                >
                  Cancelar
                </button>
                <button
